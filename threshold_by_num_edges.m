...
while (i < imax  && abs(b-a)/2 > TOL )
    %fprintf(2,' %g < T < %g %g\n',b,a,b-a);
    c = (a + b)/2;
    if ( (number_of_edges(threshold_absolute(A,c)~=0) -1 == 0) || (b-a)/2 < TOL )
        threshold = c;
    end
    if sign(number_of_edges(threshold_absolute(A,c)~=0)-1) == sign(number_of_edges(threshold_absolute(A,a)~=0)-1)
        a = c;
    else
        b = c;
    end
